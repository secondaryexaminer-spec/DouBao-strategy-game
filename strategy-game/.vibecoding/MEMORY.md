...
## 踩坑与根因（按时间倒序）
- **2026-09-08 · 队伍系统 UI 修复（6 队+历史队名）**：TEAMS ['A'..'E']→['A'..'F']；新增 TEAM_NAMES（A汉萨同盟/B拜占庭帝国/C条顿骑士团国/D莫斯科大公国/E奥斯曼帝国/F帖木儿帝国，全历史真实名，避开 5 联盟名）；UI"组别"列→"队伍"、h2"玩家与联盟"→"玩家与队伍"；下拉/侧栏/胜负播报全用队名（`teamName(t)` helper，未知字母回退 `t组`）；**236 行观战胜者正则同步改**：`^(\S+)\s*组`→`^(.*?)\s*组`+TEAM_NAMES 反查（旧档字母/新文本队名都兼容）。sim 的 teams 是独立硬编码（run.js ['B','C','A']、harness.js ['B','C','A','D','E']），不依赖 TEAMS，seed777 逐项与基线一致。队=同盟分组，独立于联盟/国家（玩家+AI 自由分配）。
